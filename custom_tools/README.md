...
```bash
# Execute single approved entry (requires DRY_RUN=false)
...
# Dry run (default) - simulates without sending
python -m custom_tools.mint_executor --id 1
```

> **Important:** `mint_executor` will ONLY execute entries with
> `status=approved`. If the entry is still `pending`, it will refuse.
